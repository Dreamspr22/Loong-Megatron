...
"""MockGatedDeltaNet - XPU mock for GatedDeltaNet"""
...
class MockGatedDeltaNet(HuggingFaceModule):
    """Mock Gated Delta Net (GDN) layer class for XPU.
    ...
    def forward(
        ...
        *,
        ...
        if cu_seqlens is not None and len(cu_seqlens) > 2:
            # Packing: insert (k-1) zeros between sequences to prevent
            # cross-boundary leakage, then use plain conv1d.
            ...
            # Vectorized: compute padded position for each original token
            # seq_indices[i] = which sequence token i belongs to
            positions = torch.arange(T, device=cu_seqlens.device)
            seq_indices = torch.searchsorted(cu_seqlens[1:], positions, right=True)
            # padded_pos[i] = i + seq_indices[i] * pad_len
            padded_positions = positions + seq_indices * pad_len

            # Scatter into zero-initialized padded tensor
            total_padded_len = T + (N - 1) * pad_len
            qkv_padded = qkv.new_zeros(qkv.shape[0], total_padded_len, D)
            scatter_idx = padded_positions.view(1, T, 1).expand_as(qkv)
            qkv_padded.scatter_(1, scatter_idx, qkv)

            # Causal conv1d
            qkv_padded = qkv_padded.transpose(1, 2)                         # [B, D, T']
            qkv_padded = self.conv1d(qkv_padded)[..., :total_padded_len]     # truncate right
            qkv_padded = self.act_fn(qkv_padded)
            qkv_padded = qkv_padded.transpose(1, 2)                         # [B, T', D]

            # Gather original positions from padded output
            qkv = qkv_padded[:, padded_positions, :]

        elif (causal_conv1d is None):
            qkv = self.act_fn(self.conv1d(qkv.transpose(1, 2).contiguous())[..., :hidden_states.shape[1]])
...
# ---------------------------------------------------------------------------
# GatedDeltaNet mock via builtins.__import__ hook
# ---------------------------------------------------------------------------

_GDN_TARGET_MODULE = 'loongforge.models.foundation.qwen3_next.gated_deltanet'
_GDN_ORIGINAL_IMPORT = None
_GDN_MOCK_APPLIED = False


def _gdn_patched_import(name, globals=None, locals=None, fromlist=(), level=0):
    global _GDN_MOCK_APPLIED

    module = _GDN_ORIGINAL_IMPORT(name, globals, locals, fromlist, level)

    if not _GDN_MOCK_APPLIED and _GDN_TARGET_MODULE in sys.modules:
        target_mod = sys.modules[_GDN_TARGET_MODULE]
        if hasattr(target_mod, 'GatedDeltaNet'):
            _GDN_MOCK_APPLIED = True
            try:
                origin = target_mod.GatedDeltaNet
                target_mod.GatedDeltaNet = MockGatedDeltaNet
                target_mod.__origin_GatedDeltaNet = origin
                print(f'[INFO] GatedDeltaNet replaced with MockGatedDeltaNet '
                      f'(via __import__ hook)')
            except Exception as e:
                print(f'[WARN] GatedDeltaNet mock failed: {e}')
            builtins.__import__ = _GDN_ORIGINAL_IMPORT

    return module


def _install_gdn_mock_hook():
    global _GDN_ORIGINAL_IMPORT
    if _GDN_ORIGINAL_IMPORT is not None:
        return
    _GDN_ORIGINAL_IMPORT = builtins.__import__
    builtins.__import__ = _gdn_patched_import
