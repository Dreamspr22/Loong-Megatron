...
def save_checkpoint(iteration, model, optimizer, opt_param_scheduler, num_floating_point_operations_so_far,
                    ...
                    train_data_iterator=None, preprocess_common_state_dict_fn=None, release=False, save_arg='save'):
    ...
    if not torch.distributed.is_initialized() \
        ...
        if ckpt_type == CheckpointType.LOCAL:
            ...
            def iter_finalize_fn():
                ...
                print_rank_0(f'  successfully saved checkpoint from iteration {int(iteration):7d} to {save_dir} '
                ...
                def delete_checkpoint(args, iteration_to_delete):
                    checkpoint_name = get_checkpoint_name(save_dir, iteration=iteration_to_delete,
                                                          return_base_dir=True)
                    try:
                ...
                if save_retain_interval is not None:
                    if prev_iteration > 0 and prev_iteration != iteration and prev_iteration % save_retain_interval != 0:
                        checkpoint_name = get_checkpoint_name(save_dir, iteration=prev_iteration,
                                                              return_base_dir=True)
...
def load_checkpoint(ddp_model, optimizer, opt_param_scheduler, load_arg='load', strict=True,
    ...
    if not release and not args.finetune and not args.no_load_optim:
        try:
            ...
            if args.use_distributed_optimizer and not is_torch_dist and \
                ckpt_format not in ["torch_dcp", "fsdp_dtensor"] and optimizer is not None:
